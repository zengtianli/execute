...
do
    ...
    # 检查文件是否为CSV文件
    if ! check_file_extension "$FILE_PATH" "csv"; then
        show_warning "跳过: $(basename "$FILE_PATH") - 不是 CSV 文件"
        continue
    fi
    ...
    # 显示处理信息
    show_processing "正在将 $(basename "$FILE_PATH") 转换为 XLSX 格式..."
    
    # 执行Python脚本处理单个文件
    if "$PYTHON_PATH" "$CONVERT_CSV_TO_XLSX" "$FILE_PATH"; then
        show_success "已将 $(basename "$FILE_PATH") 转换为 XLSX 格式"
...
if [ $TOTAL_COUNT -eq 0 ]; then
    ...
    show_success "已成功转换所有 $SUCCESS_COUNT 个 CSV 文件到 XLSX 格式"
else
    show_warning "已转换 $SUCCESS_COUNT/$TOTAL_COUNT 个 CSV 文件到 XLSX 格式"
fi
